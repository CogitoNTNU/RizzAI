...
def get_interests() -> list[str]:
    ...
    try:
        ...
        raise Exception("Could not find interests details") from e
...
def scrape_website(
    profiles_to_scrape: int = 10, like_strategy: Callable[[], None] = _give_nonono
) -> None:
    """Main function to scrape the website.

    Args:
        profiles_to_scrape (int): Number of profiles to scrape.
        like_strategy (Callable[[], None]): Function to call for liking or disliking a profile.
    """
    try:
        for _ in range(profiles_to_scrape):
...
def main() -> None:
    ...
    profiles_to_scrape = 40
    want_more = True
    while want_more and profiles_to_scrape > 0:
        try:
            scrape_website(
                profiles_to_scrape=profiles_to_scrape, like_strategy=LikeStrategy.like
            )
            profiles_to_scrape -= 1
        except Exception as e:
            print("An error occurred during scraping:", e)
